...
if (isset($_GET["id"])) {
    ...
    $stmt_art = $mysqli->query("SELECT * FROM article WHERE article_id = $article_id");
    $article = $stmt_art->fetch_array();

    // Get categories Data to display
    $stmt_cat = $mysqli->query("SELECT category_id, category_name FROM category");
}
                ...
                <div class="col-lg-12 mb-4">
                    ...
                    <form action="/process/article_process.php" method="POST" enctype="multipart/form-data">
                        <div class="form-group">
                            <input type="hidden" name="id" class="form-control" value="<?= $article["article_id"] ?>" placeholder="ID" class="form-control" required>
                        ...
                        <div class="my-2" style="width: 100px;">
                            <img class="h-auto" src="/news/img/article/<?= str_replace('../news/img/article/', '', $article['article_image']) ?>" alt="" style="width: 100px;">
                        </div>
                        <br>
                        ...
                        <div class="form-group">
                            ...
                            <select class="custom-select" name="arCategory" id="arCategory">
                                ...
                                <?php while ($row = $stmt_cat->fetch_assoc()) : ?>
                        ...
                        <div class="form-group">
                            ...
                            <input type="text" class="form-control" name="arKeywords" id="arKeywords" value="<?= $article["seo_keywords"] ?>">
                        </div>

                        <div class="form-group">
                            <label for="arDescription">Short description (Recommended to use words between 50 and 160)</label>

                            <textarea class="form-control" name="arDescription" id="arDescription" rows="3"><?= $article["seo_description"] ?></textarea>
                        </div>

                        <div class="text-center">
                            <button type="submit" name="update_article" class="btn btn-admin btn-xl">Update</button>
                        </div>
